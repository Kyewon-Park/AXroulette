...
export class Box2dPhysics implements IPhysics {
  ...
  setMarbleGhost(id: number, value: boolean): void {
    ...
    while (fixture) {
      const currentFixture = fixture as any;
      currentFixture.SetSensor(value);
      if (typeof currentFixture.Refilter === 'function') {
        currentFixture.Refilter();
      }
      fixture = currentFixture.GetNext();
    }
    body.SetAwake(true);
  }

  setMarbleTransform(id: number, position: VectorLike, angle = 0): void {
    const body = this.marbleMap[id];
    if (!body) return;
    body.SetTransform(new this.Box2D.b2Vec2(position.x, position.y), angle);
    body.SetAwake(true);
  }
  ...
  setMarbleVelocity(id: number, velocity: VectorLike): void {
    const body = this.marbleMap[id];
    if (!body) return;
    body.SetLinearVelocity(new this.Box2D.b2Vec2(velocity.x, velocity.y));
    body.SetAwake(true);
  }
